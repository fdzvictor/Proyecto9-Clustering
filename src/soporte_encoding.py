...
class TestEstadisticos:
    ...
    def detectar_orden_binaria (self):
        diccionario_diferencias = {}
        for col in self.columna_categorica:
            df_cross_tab = pd.crosstab(self.dataframe[col], self.dataframe[self.variable_respuesta])
        
            print (f"Esta es la tabla de contingencia para {col}, con la variable respuesta {self.variable_respuesta}")
            print(df_cross_tab)
            ...
            if p < 0.05:
                ...
                print("-----")
                diccionario_diferencias[col] = 1
            else:
                print("La variable categórica NO TIENE ORDEN")
                print("-----")
                diccionario_diferencias[col] = 0
...
class Encoding:
    ...
    def target_encoding(self):
        ...
        """
        # Extraer las columnas a codificar bajo la clave 'target'
        col_encode = self.diccionario_encoding.get("target", [])
    
        if col_encode:
            # Instanciar TargetEncoder
            target_encoder = TargetEncoder(cols=col_encode)
        
            # Aplicar Target Encoding a las columnas especificadas
            self.dataframe[col_encode] = target_encoder.fit_transform(self.dataframe[col_encode], self.dataframe[self.variable_respuesta])
